...
import static com.procurement.orchestrator.domain.commands.DossierCommandType.EXTEND_SUBMISSION_PERIOD;

@Component
public class DossierExtendSubmissionPeriod implements JavaDelegate {

    private static final Logger LOG = LoggerFactory.getLogger(DossierExtendSubmissionPeriod.class);
    ...
    public DossierExtendSubmissionPeriod(final DossierRestClient dossierRestClient,
    ...
    public void execute(final DelegateExecution execution) throws Exception {
        ...
        final JsonNode commandMessage = processService.getCommandMessage(EXTEND_SUBMISSION_PERIOD, context, jsonUtil.empty());
    ...
    private JsonNode update(final JsonNode jsonData, final JsonNode responseData, final String processId) {
        try {
            final ObjectNode preQualificationNode = jsonUtil.createObjectNode();
            preQualificationNode.set("period", responseData.get("period"));

            final ObjectNode main = (ObjectNode) jsonData;
            main.set("preQualification", preQualificationNode);
