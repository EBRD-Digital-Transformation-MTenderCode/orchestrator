...
class HttpSubmissionClient(private val webClient: WebClient, properties: ComponentProperties.Component) :
    ...
    ): Result<Reply<CreateInvitationsAction.Result>, Fail.Incident> = webClient.call(
        url = url,
        command = SubmissionCommands.CreateInvitations.build(id = id, params = params),
        target = SubmissionCommands.CreateInvitations.target
    )

    override suspend fun finalizeBidsByAwards(
        id: CommandId,
        params: FinalizeBidsByAwardsAction.Params
    ): Result<Reply<FinalizeBidsByAwardsAction.Result>, Fail.Incident> = webClient.call(
        url = url,
        command = SubmissionCommands.FinalizeBidsByAwards.build(id = id, params = params),
        target = SubmissionCommands.FinalizeBidsByAwards.target
    )
