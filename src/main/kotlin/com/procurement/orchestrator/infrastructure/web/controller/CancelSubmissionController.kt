...
@RestController
class CancelSubmissionController(
    private val logger: Logger,
    private val processLauncher: ProcessLauncher
) {

    companion object {
        private const val PROCESS_NAME = "withdrawSubmission"
    }

    @PostMapping("/cancel/submission/{cpid}/{ocid}/{submissionId}")
    fun create(
        servlet: HttpServletRequest,
        @PathVariable cpid: String,
        @PathVariable ocid: String,
        @PathVariable submissionId: String
    ): ResponseEntity<String> = perform(servlet = servlet, cpid = cpid, ocid = ocid, submissionId = submissionId)
