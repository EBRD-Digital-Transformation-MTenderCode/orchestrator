...
@RestController
class CompleteSourcingController(
    private val logger: Logger,
    private val processLauncher: ProcessLauncher
) {

    companion object {
        private const val PROCESS_NAME = "completeSourcing"
    }

    @PostMapping("/complete/sourcing/{cpid}/{ocid}/{lotId}")
    fun completeSourcing(
        servlet: HttpServletRequest,
        @PathVariable cpid: String,
        @PathVariable ocid: String,
        @PathVariable lotId: String
    ): ResponseEntity<String> = perform(servlet = servlet, cpid = cpid, ocid = ocid, id = lotId)
    ...
    private fun buildRequest(
        ...
        return PlatformRequest(
            ...
            context = PlatformRequest.Context(
                ...
                relatedProcess = PlatformRequest.Context.RelatedProcess(cpid = verifiedCpid)
            ),
            payload = ""
        ).asSuccess()
    }
