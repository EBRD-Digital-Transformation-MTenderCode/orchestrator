...
@RestController
class WithdrawBidController(
    private val logger: Logger,
    private val processLauncher: ProcessLauncher
) {

    companion object {
        private const val PROCESS_NAME = "withdrawBid"
    }

    @PostMapping("/cancel/bid/{cpid}/{ocid}/{bidId}")
    fun withdrawBid(
        servlet: HttpServletRequest,
        @PathVariable cpid: String,
        @PathVariable ocid: String,
        @PathVariable bidId: String
    ): ResponseEntity<String> = perform(servlet = servlet, cpid = cpid, ocid = ocid, id = bidId)
